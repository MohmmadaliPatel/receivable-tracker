...
export default function DashboardOverview() {
  ...
  const fetchStats = async () => {
    try {
      const [sendersRes, rulesRes, forwardersRes] = await Promise.all([
        fetch('/api/senders'),
      ...
      if (sendersRes.ok) {
        ...
        setStats(prev => ({ ...prev, totalSenders: senders.length }));
        
        senders.forEach((sender: any) => {
          totalEmails += sender.emailTrackings?.length || 0;
          forwardedEmails += sender.emailTrackings?.filter((e: any) => e.isForwarded).length || 0;
        });
      }
      ...
      if (forwardersRes.ok) {
        ...
        const totalForwarders = (forwardersData.forwarders || []).length;
        setStats(prev => ({ ...prev, totalForwarders }));
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.totalSenders}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Total Forwarders</p>
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.totalForwarders}</p>
            </div>
            <div className="text-4xl">📤</div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          >
            ...
            <h3 className="font-semibold text-gray-900">Manage Senders</h3>
            <p className="text-sm text-gray-600 mt-1">Add and manage email senders</p>
          </Link>
          ...
          >
            <div className="text-2xl mb-2">📤</div>
            <h3 className="font-semibold text-gray-900">Forwarders</h3>
            <p className="text-sm text-gray-600 mt-1">Manage email forwarders</p>
          </Link>
          ...
          >
            <div className="text-2xl mb-2">📬</div>
            <h3 className="font-semibold text-gray-900">Forwarded Emails</h3>
            <p className="text-sm text-gray-600 mt-1">View all forwarded emails and replies</p>
          </Link>
